...
### 📍 TODO
1. Fix frontend blank page bug(done)
2. Integrate real WebSocket for live opportunities(done)
...
React blank page sudah fix, bundling Vite + Docker + Nginx sudah sinkron.
UI ArbBot Pro dari guntjong-max/guntjong-max-arb sudah terintegrasi ke minimal-ui (App.jsx baru, components & types sudah masuk).
API base URL sekarang pakai VITE_API_URL dan sementara diset ke http://217.216.35.6:3000.
Dashboard tampil, tapi layout masih perlu polishing (grid/cards, spacing, typography).
Backend:
Endpoint /api/system-health, /api/settings, /api/login-status, /api/bets belum diimplementasi penuh sehingga masih 404.
Next steps (besok):
Rapikan layout dan styling di minimal-ui/src/App.jsx dan komponen terkait (Header, AccountPanel, Configuration, Monitoring, Logs).
Setelah desain oke, implement API backend minimal supaya 404 hilang.

3. **API integration** (LOW)
   - Backend health check working
   - Real opportunities not flowing yet
   - ✅ MINIMALIST ENGINE — BACKEND API SPEC (FINAL & COMPLETE)
...
POST	/api/execute	Kirim instruksi bet ke backend (backend → worker sequence)
...
________________________________________
2️⃣ DETAILED API SPEC (Coder-Friendly)
________________________________________
✅ 1. POST /api/login
Tujuan:
...
________________________________________
7️⃣ API paling penting: /api/system-health
...
>350ms	🔴
________________________________________
8️⃣ FLOW SCAN MINIMALIST (FINAL)
Worker:
Login → Simpan cookies
Poll API 250–400ms
Kirim ke backend via WebSocket
Terima bet instruction
Eksekusi bet → kirim result → cooldown 60 detik
Backend:
...
Jika accepted → kirim leg-2
Simpan history
Update cooldown masing-masing provider
________________________________________
9️⃣ FORMAT data odds (wajib pakai)
